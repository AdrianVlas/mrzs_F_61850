...
//  count_bit = 10;
  case 120:
    ...
    if(regUst<SETPOINT_UMIN1_MIN/10 || regUst>SETPOINT_UMIN1_MAX/10) diapazon=0;
    break;
  case 121:
  ...
  case 122:
    ...
    if(regUst<SETPOINT_UMIN2_MIN/10 || regUst>SETPOINT_UMIN2_MAX/10) diapazon=0;
    break;
  case 123:
  ...
  case 126:
    ...
    if(regUst<SETPOINT_UMAX1_MIN/10 || regUst>SETPOINT_UMAX1_MAX/10) diapazon=0;
    break;
  case 127:
  ...
  case 128:
    ...
    if(regUst<SETPOINT_UMAX2_MIN/10 || regUst>SETPOINT_UMAX2_MAX/10) diapazon=0;
    break;
  case 129:
...
...
  ...
  else if (address_data == MA_DEFAULT_SETTINGS)
  {
    ...
    {
      //Для встановлення мінімальної конфігурації має бути прописано
       по певній адресі визначене число - інакше повідомляємо,
       що такої адреси взагалі не існує (примітивний метод маскування
       від несанкціонованого дослідження карти пам'яті)
