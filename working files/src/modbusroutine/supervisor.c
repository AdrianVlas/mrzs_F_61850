...
int inputPacketParser(void)
{
  ...
  if(*received_count>255) return 0;//не отвечать//слишком длинный пакет - не отвечать

  CRC_sum = 0xffff;
  for (int index = 0; index < (*received_count-2); index++) CRC_sum = AddCRC(*(inputPacket + index),CRC_sum);
  if((CRC_sum & 0xff)  != *(inputPacket+*received_count-2) ||
      (CRC_sum >> 8  ) != *(inputPacket+*received_count-1)) return 0;//не отвечать

  if(inputPacket[0] != current_settings.address) return 0;//не отвечать
